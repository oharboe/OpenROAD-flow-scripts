...
class LefTest(unittest.TestCase):
    ...
    def test_power_ground_straps_interleaved(self):
        ...
        vdd_ys = [float(m.group(2))
                  for m in re.finditer(r"RECT ([\d.]+) ([\d.]+)", vdd)]
        vss_ys = [float(m.group(2))
                  for m in re.finditer(r"RECT ([\d.]+) ([\d.]+)", vss)]
        ...
        self.assertAlmostEqual(vdd_ys[1] - vdd_ys[0],
                               self.params.pg_strap_pitch_um)

    def test_all_shapes_inside_macro(self):
        for m in re.finditer(
                r"RECT\s+([\d.]+) ([\d.]+) ([\d.]+) ([\d.]+) ;", self.lef):
    ...
    def test_obs_covers_footprint_on_all_layers(self):
        ...
        for layer in (self.params.obs_device_layers +
                      self.params.obs_metal_layers):
