...
def run(verilog: list[Path], memories_files: list[Path], platform: str,
        out_dir: Path, json_path: Path) -> int:
    try:
    ...
    converted = sorted((m for m in memories if m.idiomatic),
                       key=lambda m: m.name)
    try:
    ...
    for mem in converted:
        (out_dir / f"{mem.name}.lib").write_text(liberty.emit_lib(
            mem, params))
    ...
    (out_dir / "blackboxes.txt").write_text("".join(
        f"{m.name}\n" for m in converted))
...
def main(argv: list[str] | None = None) -> int:
    ...
    return run(args.verilog, args.memories, args.platform, args.out_dir,
               args.json)
