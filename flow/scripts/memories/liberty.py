...
def ordered_port_groups(
        mem: schema.Memory) -> list[tuple[str, list[schema.Pin]]]:
    ...
    for port_id in sorted(groups, key=port_key):
        pins = sorted(
            groups[port_id],
            key=lambda p:
            (_FUNCTION_ORDER[p.function], _subword_index(p.name), p.name))
        out.append((port_id, pins))
    return out


def _is_bus(pin: schema.Pin) -> bool:
    return pin.width > 1 and pin.function in ("addr", "data_in", "data_out",
                                              "mask")
...
def predict_access_ps(rows: int, bits: int, ports_total: int,
                      tech_nm: int) -> float:
...
def predict_clk_period_ps(rows: int, bits: int, ports_total: int,
                          tech_nm: int) -> float:
    return max(
        predict_access_ps(rows, bits, ports_total, tech_nm) +
        predict_setup_ps(tech_nm) + 30.0, 150.0 * _scale_tech(tech_nm))
...
def predict_leakage_pw(rows: int, bits: int, ports_total: int,
                       tech_nm: int) -> float:
    s = _scale_tech(tech_nm)
    rows_eff = max(rows, 2)
    bits_eff = max(bits, 1)
...
def emit_lib(mem: schema.Memory,
             params: PdkParams,
             *,
             ck_insertion_ps: float | None = None) -> str:
    ...
    clk_period_ps = predict_clk_period_ps(mem.rows, mem.bits, ports_total,
                                          tech_nm)
    ck_ins_ps = (ck_insertion_ps if ck_insertion_ps is not None else
                 predict_ck_insertion_ps(tech_nm))
