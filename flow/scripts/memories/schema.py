...
def _pin_from_dict(d: dict) -> Pin:
    ...
    if pin.function not in PIN_FUNCTIONS:
        raise SchemaError(f"pin {pin.name}: unknown function "
...
def memory_from_dict(d: dict) -> Memory:
    ...
    for key in ("rows", "bits", "addr_w", "read_ports", "write_ports",
                "rw_ports", "mask_lanes"):
        if key in d:
...
def dump(memories: list[Memory], path: Path, platform: str) -> None:
    doc = {
        ...
        "platform": platform,
        "memories": [m.to_dict() for m in sorted(memories,
                                                 key=lambda m: m.name)],
    }
...
def validate_emittable(mem: Memory) -> None:
    ...
    for port_id in port_ids:
        ...
        if "clk" not in functions:
            raise SchemaError(
                f"memory {mem.name}: port {port_id} has no clk pin")
