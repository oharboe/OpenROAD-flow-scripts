...
Coefficients are anchored to published 7 nm SP-SRAM IP (Suzuki et al.,
ISSCC 2018, "A 6T-SRAM in 7 nm FinFET CMOS" — ~0.027 µm² per bit cell;
TSMC N7 SRAM compiler datasheet figures for peripheral overhead). The
...
def sram_area_um2(rows: int,
                  cols: int,
                  read_ports: int = 1,
                  write_ports: int = 1,
                  tech_nm: int = 7,
                  write_mask_lanes: int = 0) -> float:
    ...
    if read_ports < 0 or write_ports < 0:
        raise ValueError(f"port counts must be >= 0; got "
    ...
    bits = rows * cols
    port_factor = (1.0 + _PORT_FACTOR_READ * max(read_ports - 1, 0) +
    ...
    peripheral += (write_mask_lanes * _WRITE_MASK_PER_LANE_UM2 *
                   max(write_ports, 0))
...
def sram_dimensions_um(area_um2: float,
                       aspect_ratio: float = 0.5) -> tuple[float, float]:
