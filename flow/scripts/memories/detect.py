...
def classify_module(name: str, ports: list[tuple[str, str, int]],
                    source_file: str = "") -> schema.Memory | None:
    ...
    for port_id, group in groups.items():
        ...
        if port_id.startswith("RW"):
            ...
            n_write += 1
        addr_w = max(addr_w,
                     max(p.width for p in group if p.function == "addr"))
        for function in ("data_in", "data_out"):
            bits = max(
                bits,
                sum(p.width for p in group if p.function == function))
        mask_lanes = max(
