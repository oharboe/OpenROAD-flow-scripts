...
class SchemaTest(unittest.TestCase):
    ...
    def test_dump_is_sorted_and_versioned(self):
        with tempfile.TemporaryDirectory() as d:
            path = Path(d) / "memories.json"
            schema.dump([sample_memory("zz"), sample_memory("aa")],
                        path,
                        platform="asap7")
    ...
    def test_merge_partial_override(self):
        ...
        override = schema.Memory(name="mem_64x32",
                                 idiomatic=True,
                                 reason="forced: no behavioral fallback")
    ...
    def test_merge_new_entry_taken_whole(self):
        ...
        self.assertEqual(sorted(m.name for m in merged),
                         ["mem_64x32", "mem_extra"])
